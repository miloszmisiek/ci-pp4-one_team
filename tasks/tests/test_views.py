...
class TestViews(TestCase):
    ...
    def setUp(self):
        ...
        self.tasks = Task.objects.all().exclude(updated_on=CLEAR_UPDATED, status=1)
    ...
    def test_get_profile_home_view_clear_prolonged_completed(self):
        """Test tasks home page view - prolonged tasks excluded from view"""
        self.client.force_login(self.user_senior)
        self.test_task.updated_on = CLEAR_UPDATED
    ...
    def test_add_task_view_junior_get(self):
        ...
        self.assertTrue(response.context['form'].fields["assigned_to"].disabled)
        self.assertEquals(response.context['form'].initial['assigned_to'], self.user_junior.id)
        self.assertEquals(response.context['form'].assigned_to, MASTER_EXCLUDED)

    # def test_add_task_view_junior_post(self):
    #     """Test add task view - junior get view"""
    #     self.client.force_login(self.user_junior)
    #     self.data = {
    #         'title': 'task-title',
